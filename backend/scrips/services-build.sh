...
function createSeviceImage () {
    ...
    docker run --rm -v $GOPATH:/go golang:$alpineVersion sh -c " apk add --no-cache gcc musl-dev && cd /go/src/github.com/MihaiLupoiu/money-transferring-simulation/backend/services/${_serviceName} && go build -o ${_serviceName}.alpine"
    ret=$?
    if [ $ret -ne 0 ]; then
        exit -1
    fi
